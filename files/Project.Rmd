...
dt_matches[matchId=="h4GSow3N",c("score")]="1:2"
  
dt_matches[matchId=="WlSJmaYA",c("score")]="1:2"
...
dt_season1<-dt_dataset[date>="2018-08-01"& date<="2019-01-03"][bookmaker=="Pinnacle"]
...
feature_extract = function(data){
  ...
  #homeL5WinRatio=as.numeric(mean(fixture[Team=="bournemouth"][1:3,c("WinProbratio")]$WinProbratio,na.rm = TRUE))
...
#model_dataall<-model_data[,c("MatchResult","homeprob","awayprob","drawprob","HomeTotalGrade","AwayTotalGrade","homeL3SGoal","homeL3CGoal","awayL3SGoal", "awayL3CGoal","homeL3wincount","homeL3defeatcount","awayL3wincount", "awayL3defeatcount","homeOldRps","awayOldRps","homeWinRatioInHome", "awayWinRatioInAway","awayL5avggrade",     "homeinhomeOldrps","awayinawayOldrps","homeL5WinRatio","awayL5WinRatio","homeL5rps","awayL5rps","homeL5avggrade","Is.special","date" )]

#model_dataall<-na.omit(model_dataall)        
  


#model_data<-model_data[,c("matchId","leagueId","odd1","date","MatchResult","homeprob","awayprob","drawprob","homeL5rps","awayL5rps","AwayTotalGrade","HomeTotalGrade","Is.special","homeWinRatioInHome","awayWinRatioInAway")]
                          


##model_data[,c("matchId","leagueId","odd1","date","MatchResult","homeprob","awayprob","drawprob","homeL5rps","awayL5rps","AwayTotalGrade","HomeTotalGrade","Is.special"] son hafta
...
#model_data<-model_data[,c("matchId","leagueId","odd1","date","MatchResult","homeprob","drawprob","homeOldRps","awayOldRps","homeWinRatioInHome","awayWinRatioInAway","AwayTotalGrade","HomeTotalGrade","Is.special")]
 ...
 train.data<- model_data[date<"2018-12-31"] # 70% data goes in here 2018-12-03
 test.data<- model_data[date>"2018-12-31"]    #& date<"2018-12-24"]
...
correlationMatrix <- cor(model_dataall[,2:27])
...
rfbank <- train(MatchResult ~ ., data=model_dataall, method="rf", metric="Accuracy",ntree =500, trControl=control,tuneLength=5)
                
   

print(rfbank)

a<-rfbank$finalModel$importance
             


fit_rf = randomForest(MatchResult~., data=model_dataall)





datafeature<-model_data[,5:16]

datafeature<-na.omit(datafeature)

datafeature[,`:=`(MatchResultBinary = ifelse(MatchResult=="Home",1,ifelse(MatchResult=="Tie",2,ifelse(MatchResult=="Away",3,0))))]

datafeature<-datafeature[,2:12]
